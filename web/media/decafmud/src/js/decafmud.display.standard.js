 ...
 */
var alert_snd = new Audio("decafmud/media/alert.wav"); // buffers automatically when created
...
var Display = function(decaf, ui, disp) {
	...
	this.message("Winter's Oasis is loading...");
...
Display.prototype.processData = function() {
	...
	this.inbuf = [];
	if ( ! this.decaf.alerted ) {
            alert_snd.play();
            alert_snd.currentTime = 0;
            decaf.alerted = true;
            decaf.blink = false;
            window_blinker = setInterval( function () {
                if (! decaf.blink) {
	...
	var span = document.createElement('span');
        ...
        console.log(text);
        var linked_up  = text.replace(/((http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?)/ig, '<a href="' + "$1" + '" target="_blank">' + "$1" + '</a>');
        console.log(linked_up);
        span.innerHTML = linked_up;
	this.shouldScroll();
	this.display.appendChild(span);
	this.doScroll();
